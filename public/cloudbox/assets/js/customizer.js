(function (jQuery) {
    ...
    localDark = localStorage.getItem("dark");
    if(localDark !== "null"){
        if(localDark !== "null"){
            changeMode(localDark);
        }
    } else {
        if ($("body").hasClass("dark")) {
            changeMode("true");
    ...
    function updateLocalStorage(theme) {
        // Simpan tema di sessionStorage dan localStorage
...
$(function () {
    ...
    $(document).on("click", function (evt) {
        if (
            $(evt.target).closest(".select-dropdown > .selet-caption")
                .length === 0
        ) {
            $(".select-dropdown").removeClass("open");
        }
